...
class Client extends Dispatcher {
  constructor (url, {
    ...
    if (connector != null && typeof connect !== 'object') {
      throw new InvalidArgumentError('connect must be a Connector')
    }

    this[kUrl] = util.parseOrigin(url)
    this[kConnector] = connector || new Connector({ tls, url: this[kUrl], socketPath })
    this[kSocket] = null
...
function detachSocket (socket) {
  socket[kConnector].detach(socket)
  socket[kConnector] = null
...
function connect (client) {
  ...
  const socket = client[kConnector].connect({
    servername: client[kHost] && util.getServerName(client[kHost])
  }, onSocketConnect)

  client[kSocket] = socket

  socket[kConnector] = client[kConnector]
...
function _resume (client, sync) {
  while (true) {
    ...
    if (client[kUrl].protocol === 'https:' && client[kHost] !== request.host) {
      ...
      const servername = client[kHost]
        ? util.getServerName(client[kHost])
        : client[kConnector].servername

      assert(socket.servername)

      if (socket.servername !== servername) {
        util.destroy(socket, new Error('servername changed'))
      }
    }
