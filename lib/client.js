...
function getServerName (client, host, tls) {
  return (
...
function defaultConnect (client, { protocol, socketPath, hostname, port, servername, tls: tlsOpts }) {
  let socket
  if (protocol === 'https:') {
    tlsOpts = {
      ...tlsOpts,
...
class Client extends Dispatcher {
  constructor (url, {
    ...
    if (connect != null && typeof connect !== 'function') {
      throw new InvalidArgumentError('connect must be function')
    }

    this[kFactory] = connect || ((opts) => defaultConnect(this, { tls, ...opts }))
    this[kSocket] = null
    ...
    this[kTLSServerName] = getServerName(this, null, tls)
    this[kHost] = null
    this[kOnDestroyed] = []
...
function connect (client) {
  ...
  const socket = client[kFactory]({
    protocol,
    socketPath: client[kSocketPath],
    hostname,
    port,
