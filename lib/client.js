...
try {
  ...
  `
  new WebAssembly.Module(simdWASM) // eslint-disable-line no-new
  hasSimd = true
} catch (err) {
  hasSimd = false
}

const WASM_PATH = hasSimd ? './llhttp/llhttp_simd.wasm' : './llhttp/llhttp.wasm'
const mod = new WebAssembly.Module(readFileSync(resolve(__dirname, WASM_PATH)))
