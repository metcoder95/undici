...
function defaultConnect (pool, { protocol, socketPath, hostname, port, servername, tls: tlsOpts }) {
  let socket
  if (protocol === 'https:') {
    tlsOpts = {
      ...tlsOpts,
    ...
    if (tlsOpts.reuseSessions !== false) {
      ...
      this.on('disconnect', (origin, targets, err) => {
        if (err.code === 'UND_ERR_INFO' && err.message === 'upgrade') {
          socket
            .removeListener('session', onSession)
...
class Pool extends Dispatcher {
  constructor (origin, { connections, factory, ...options } = {}) {
    super()
    ...
    this[kTLSSession] = options.tls && options.tls.session
    this[kClients] = []
    this[kNeedDrain] = false
    this[kQueued] = 0
    this[kFactory] = factory || ((origin, opts) => new Client(origin, { connect: (opts) => defaultConnect(pool, opts), ...opts }))
